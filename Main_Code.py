...
table_colors = session.table("catalog_for_website").select(col("color_or_style"))
##pd_colors = table_colors.to_pandas()

# Oyt the list of colors into a drop list selector 
option = st.selectbox('Pick a sweatsuit color or style:', table_colors)

# We'll build the image caption now, since we can
product_caption = st.write('Our warm, comfortable, ',option, ' sweatsuit !')

# use the color selected to go back and get all the info from the database
table_prod_data = session.sql(f"select file_name, price, size_list, upsell_product_desc, file_url from catalog_for_website where color_or_style = '{option}';")
